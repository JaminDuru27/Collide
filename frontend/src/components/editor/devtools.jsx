...
export function DevTools({mode, play, collide, setPlay, fullscreen, setFullscreen}){
    return (
        <motion.div 
            ...
            >   
                <div
                onClick={()=>{
                    collide[`current`].setdevstate(collide[`current`].devstate === `play`?`pause`:`play`)
                    setPlay(p=>collide[`current`].devstate)
                }}
                
                className=" items-center justify-center cursor-pointer flex w-5 h-5 rounded-sm">
                    {(collide[`current`].devstate === `play`)?<FaPlay title='Playing (Space to change)'/>:<FaPause title='Paused (Space to change)'/>}
